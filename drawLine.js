const fs = require('fs');
const readline = require('readline');//Read file line by line;
const express = require('express');
const path = require('path');//A utility module that deals with paths.
const bodyParser = require('body-parser');
const app = express();//express contains a bunch of methods;
const open = require('open');//Open website in default browser;
const vscode = require('vscode');
const url = require('url');

module.exports = function(context) {
  context.subscriptions.push(vscode.commands.registerCommand('extension.bugReport', function () {
    var pointObj;
    var pointMap = new Map();//make FilePath as key, Lines as value.
    var errorMap = new Map();
    //var vsrootPath = vscode.workspace.rootPath;
    var workspacePath = vscode.workspace.rootPath.split("bullet-2.81-rev2613-code-map-test")[0];
    var vsrootPath = path.join(workspacePath,"bullet-2.81-rev2613-code-map-test")
    var vscodefile = path.join(vsrootPath,".vscode");
    var testPath = path.join(vscodefile,"test.json");// /WORKSPACE/bullet-2.81-rev2613-code-map-test/.vscode/test.json';
    vscode.workspace.openTextDocument(vscode.Uri.file(testPath)).then(document => {
      const content = document.getText();//Get the content of the 'test.json' file.
      pointObj = eval("("+content+")");//convert json to js object

      for(var i=0; i<pointObj.DrawLines.length; i++){
        var key = pointObj.DrawLines[i].FilePath;//Key.
        var points = pointObj.DrawLines[i].Lines;//Points in Lines.
        var errors = pointObj.DrawLines[i].Errors;
        //console.log(errors);
        if(pointMap.has(key)){//if the key is same, original value should plus new value.
          var oriPoints = pointMap.get(key);
          var pointSplit = [{row: -1, column: -1}];//use (-1,-1) as the separator between the different lines in the same file
          var totalPoints = (oriPoints.concat(pointSplit)).concat(points);
          pointMap.set(key,totalPoints);
    
          var oriErrors = errorMap.get(key);
          var totalErrors = oriErrors.concat(errors);
          errorMap.set(key,totalErrors);
        }else{
          pointMap.set(key,points);
          errorMap.set(key,errors);
        }
      }
    
      for (var [key, value] of pointMap) {
        //console.log(key + ' = ' + value[0].row);//console log test
        value = [value,errorMap.get(key)];
        var workspacePath = vscode.workspace.rootPath.split("bullet-2.81-rev2613-code-map-test")[0];
        var errorFilePath = path.join(workspacePath,key);
        console.log("QQQQQQQQQQQQQQQ: "+errorFilePath);
        drawLine(errorFilePath,value);
      }
    });

// The code below was used in nodejs, and cannot be used in this vscode extension due to the function readFile.
// fs.readFile("test.json",'utf8',function (err, data) {
//   if(err) console.log(err);
//   pointObj = eval("("+data+")");//convert json to js object

//   for(var i=0; i<pointObj.DrawLines.length; i++){
//     var key = pointObj.DrawLines[i].FilePath;//Key.
//     var points = pointObj.DrawLines[i].Lines;//Points in Lines.
//     var errors = pointObj.DrawLines[i].Errors;
//     //console.log(errors);
//     if(pointMap.has(key)){//if the key is same, original value should plus new value.
//       var oriPoints = pointMap.get(key);
//       var pointSplit = [{row: -1, column: -1}];//use (-1,-1) as the separator between the different lines in the same file
//       var totalPoints = (oriPoints.concat(pointSplit)).concat(points);
//       pointMap.set(key,totalPoints);

//       var oriErrors = errorMap.get(key);
//       var totalErrors = oriErrors.concat(errors);
//       errorMap.set(key,totalErrors);
//     }else{
//       pointMap.set(key,points);
//       errorMap.set(key,errors);
//     }
//   }

//   for (var [key, value] of pointMap) {
//     //console.log(key + ' = ' + value[0].row);//console log test
//     value = [value,errorMap.get(key)];
//     drawLine(key,value);
//   }
// });

    var flag = true;

    function drawLine(key, value){
      let filepath = path.join(key)//Path of the C or C++ file
      let input = fs.createReadStream(filepath)
      console.log("Test1: drawLine");
      var content = "";//The content of the C or C++ file
      var lineNumber = 1;//Count numbers for lines of the content
      var rl = readline.createInterface({
        input: input
      });
      console.log("Test2: drawLine");
      rl.on('line', (line) => {
        var space = "";
          if(lineNumber<=9){
            space = "    ";
          }else{
            space = "   ";
          }
          content += lineNumber++ +space+ line+ '\n';// Add line number for the C or C++ code.
      });
      console.log("Test3: drawLine");
      rl.on('close', (line) => {
        var totalPoints = [{content:content}].concat(value);//Add the target file content into the value.
        pointMap.set(key,totalPoints);
        //console.log(pointMap.get(key)[0].content);//Test the forEach content 
        console.log("Test4: drawLine");
        app.use('/',express.static(path.join(__dirname,'')));
        //Allows parsing of URL-encoded forms; The extended option is set to false because there is no need to deal with any complicated objects.
        app.use(bodyParser.urlencoded({extended:false}));
        //Load HTML file via get
        app.get('/',(req,res)=>{
            res.sendFile(path.join(__dirname,'drawLine.html'));
        });
        console.log("Test5: drawLine");
        //Load HTML file via post
        app.post('/getContent',(req,res)=>{
            //console.log(req.body.fileName);//The file name
            var key = req.body.fileName;
            //console.log(pointMap.get(key));
            res.send(pointMap.get(key));
        });
        console.log("Test6: drawLine");
        if(flag){
          app.listen(3000);
          flag = false;
        }
        console.log("Test7: drawLine");
        open('http://127.0.0.1:3000/drawLine.html?mapKey='+key);
        console.log("Test8: drawLine");
      });
    }
  }));

  vscode.commands.executeCommand('extension.bugReport');
};